...
import { getRouteParams, getRouteSearchParams, Router as TanstackRouter } from './router';
...
export class LinkedQueryParamGlobalHandler {
    private _router = inject(TanstackRouter);
    /**
     * @internal
    ...
    private navigate(): Promise<boolean> {
        const navigateOptions: NavigateOptions = {
            search: this._currentKeys,
        }

        console.log(navigateOptions)
        const navigation = this._router.navigate(navigateOptions);
        if (navigation instanceof Promise) {
            return navigation.then((value) => {
...
export function linkedQueryParam<T>(
    ...
    return runInInjectionContext(injector, () => {
        ...
        const routeSearchParams = getRouteSearchParams();
        const routeParams = getRouteParams();

        console.log({ params: routeParams(), search: routeSearchParams() })
        ...
        const searchParamValue = computed(() => {
            // @ts-ignore
            return routeSearchParams()[key];
        });

        const source = signal<T>(searchParamValue() as T);

        const originalSet = source.set;

        explicitEffect([searchParamValue], ([value]) => {
